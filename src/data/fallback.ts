...
export function fallbackReport(date = fallbackDate, locale: Locale = "en"): ReportResult {
  ...
  const sessionsValue = (count: number) => label(`${count} session(s)`, `${count} 个 session`);
  ...
  return {
    markdown: [
      ...
      `${label("Parallel project ratio estimated", "并行项目比例估算")}: ${Math.round(ledger.parallelReview.parallelProjectRatio * 100)}%`,
      `${label("AI waiting / human review overlap estimated", "AI waiting / 人工 review 重叠估算")}: ${Math.round(ledger.parallelReview.aiWaitingHumanOverlapSeconds / 60)}m`,
      `${label("Review backlog estimated", "Review backlog 估算")}: ${sessionsValue(ledger.parallelReview.reviewBacklogSessionCount)}, ${Math.round(ledger.parallelReview.reviewBacklogSeconds / 60)}m`,
      `${label("Context switches", "上下文切换")}: ${ledger.parallelReview.contextSwitchCount} ${label("total", "总计")}, ${ledger.parallelReview.shortContextSwitchCount} ${label("short", "短时")}`,
      `${label("Delivery absorbed", "已吸收交付")}: ${ledger.deliveryReview.absorbedSessions}/${ledger.metrics.sessionCount}`,
      `${label("Delivery commits / dirty", "交付 commit / dirty")}: ${ledger.deliveryReview.sessionsWithCommits} ${label("committed", "已提交")}, ${ledger.deliveryReview.sessionsWithDirtyChanges} dirty`,
      `${label("PR / CI / Issue signals", "PR / CI / Issue 信号")}: ${ledger.deliveryReview.sessionsWithPr} PR, ${ledger.deliveryReview.sessionsWithCiSignal} CI/local test, ${ledger.deliveryReview.sessionsWithIssues} ${label("issue-linked", "issue 关联")}`,
      `${label("Tool calls", "工具调用")}: ${ledger.metrics.toolCallCount}`,
      `Tokens: ${ledger.metrics.tokenCount}`,
      `${label("Cost", "成本")}: $${ledger.metrics.costAmount.toFixed(4)}`,
      `${label("Value mix", "价值构成")}: ${ledger.metrics.highValueCount} ${label("high", "高价值")}, ${ledger.metrics.lowValueCount} ${label("low", "低价值")}, ${ledger.metrics.needsRepairValueCount} ${label("repair", "需修复")}, ${ledger.metrics.discardedValueCount} ${label("discarded", "已丢弃")}`,
      "",
      `## ${label("Parallel Review", "并行复盘")}`,
      `- ${label("Parallel project ratio estimated", "并行项目比例估算")}: ${Math.round(ledger.parallelReview.parallelProjectRatio * 100)}%`,
      `- ${label("AI waiting / human review overlap estimated", "AI waiting / 人工 review 重叠估算")}: ${Math.round(ledger.parallelReview.aiWaitingHumanOverlapSeconds / 60)}m`,
      `- ${label("Review backlog estimated", "Review backlog 估算")}: ${sessionsValue(ledger.parallelReview.reviewBacklogSessionCount)}, ${Math.round(ledger.parallelReview.reviewBacklogSeconds / 60)}m`,
      `- ${label("Context switches", "上下文切换")}: ${ledger.parallelReview.contextSwitchCount} ${label("total", "总计")}, ${ledger.parallelReview.shortContextSwitchCount} ${label("short", "短时")}`,
      "",
      `## ${label("Delivery Review", "交付复盘")}`,
      `- ${label("File-changing sessions", "改动文件的 sessions")}: ${ledger.deliveryReview.sessionsWithFileChanges}`,
      `- ${label("Absorbed sessions", "已吸收 sessions")}: ${ledger.deliveryReview.absorbedSessions}`,
      `- ${label("Commit signals", "Commit 信号")}: ${ledger.deliveryReview.sessionsWithCommits}`,
      `- ${label("Dirty after session", "Session 后仍 dirty")}: ${ledger.deliveryReview.sessionsWithDirtyChanges}`,
      `- ${label("Delivery Integrations", "交付集成")}: ${ledger.deliveryReview.sessionsWithPr} PR, ${ledger.deliveryReview.sessionsWithIssues} ${label("issue-linked", "issue 关联")}, ${ledger.deliveryReview.sessionsWithCiSignal} CI/local test signal(s), ${ledger.deliveryReview.mergedSessions} merged`,
      "",
      `## ${label("AI Dev Operating Review", "AI 开发运营复盘")}`,
      `- ${label("Success rate estimated", "成功率估算")}: ${Math.round(ledger.operatingReview.successRate * 100)}% (${ledger.operatingReview.successfulSessions}/${ledger.operatingReview.totalSessions})`,
      `- ${label("Cross-tool sources", "跨工具来源")}: ${ledger.operatingReview.crossToolSourceCount}, ${label("cross-projects", "跨项目")}: ${ledger.operatingReview.crossProjectCount}`,
...
export function fallbackWeeklyReport(date = fallbackDate, locale: Locale = "en"): ReportResult {
  ...
  return {
    markdown: [
      `# ${label("Weekly Report", "周报")} - ${start} ${label("to", "至")} ${end}`,
      "",
      `${t("report.fallback.sessions")}: ${ledger.metrics.sessionCount}`,
      `${t("report.fallback.projects")}: ${ledger.metrics.projectCount}`,
      `${label("Prompting", "Prompting")}: ${Math.round((ledger.metrics.promptingSecondsEstimated + ledger.metrics.promptingSecondsManual) / 60)}m`,
      `${label("AI waiting", "AI waiting")}: ${Math.round((ledger.metrics.aiWaitingSecondsEstimated + ledger.metrics.aiWaitingSecondsManual) / 60)}m`,
      `${label("Review", "Review")}: ${Math.round((ledger.metrics.reviewSecondsEstimated + ledger.metrics.reviewSecondsManual) / 60)}m`,
      `${label("Repair", "Repair")}: ${Math.round((ledger.metrics.repairSecondsEstimated + ledger.metrics.repairSecondsManual) / 60)}m`,
      `${label("Tool calls", "工具调用")}: ${ledger.metrics.toolCallCount}`,
      `Tokens: ${ledger.metrics.tokenCount}`,
      `${label("Cost", "成本")}: $${ledger.metrics.costAmount.toFixed(4)}`,
      `${label("Delivery absorbed", "已吸收交付")}: ${ledger.deliveryReview.absorbedSessions}/${ledger.metrics.sessionCount}`,
      `${label("PR / CI / Issue signals", "PR / CI / Issue 信号")}: ${ledger.deliveryReview.sessionsWithPr} PR, ${ledger.deliveryReview.sessionsWithCiSignal} CI/local test, ${ledger.deliveryReview.sessionsWithIssues} ${label("issue-linked", "issue 关联")}`,
      "",
      `## ${label("Most Valuable Sessions", "最有价值的 Sessions")}`,
      ...(topValue.length > 0 ? topValue.map(line) : [label("- No high-value sessions marked yet.", "- 还没有标记高价值 sessions。")]),
      "",
      `## ${label("Most Wasteful Sessions", "最浪费的 Sessions")}`,
      ...(waste.length > 0 ? waste.map(line) : [label("- No obvious waste sessions based on current marks.", "- 按当前标记没有明显浪费的 sessions。")]),
      "",
      `## ${label("Workflow Notes", "Workflow 备注")}`,
      label("- Review and repair are estimated until you confirm them in session detail.", "- 在 session detail 中确认前，review 和 repair 均为估算。"),
      "",
      `## ${label("Parallel Review", "并行复盘")}`,
      `- ${label("Parallel project ratio estimated", "并行项目比例估算")}: ${Math.round(ledger.parallelReview.parallelProjectRatio * 100)}%`,
      `- ${label("AI waiting / human review overlap estimated", "AI waiting / 人工 review 重叠估算")}: ${Math.round(ledger.parallelReview.aiWaitingHumanOverlapSeconds / 60)}m`,
